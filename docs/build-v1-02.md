...
POST /api/ideas
  Body: { title, one_liner, pitch, post_token, website?, turnstile_token? }
  Process:
...
- Compute a normalized `content_hash` from lowercased, whitespace-collapsed `title + one_liner + pitch`
...
- **Commit example:** `[agent-name] feat: add anonymous posting with lightweight anti-abuse guards`
...
function normalizeIdeaContent(input: {
  ...
  return `${input.title} ${input.one_liner} ${input.pitch}`
    .toLowerCase()
    .replace(/\s+/g, ' ')
    .trim();
}
...
*End of build specification v1.02. Next amendment will be `build-v1-03.md`.*
